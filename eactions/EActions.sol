...
abstract contract EXPXOwnable {
    function setController(address controller) external virtual;
}

abstract contract AbstractPool is BEP20, EXPXOwnable {
...
abstract contract EPool is AbstractPool {
...
abstract contract EFactory {
    function newEPool() external virtual returns (EPool);
}
...
abstract contract ConfigurableRightsPool is AbstractPool {
    ...
    function ePool() external view virtual returns (EPool);
}
...
contract EActions {
    ...
    function createSmartPool(
        ...
        for (uint i = 0; i < poolParams.constituentTokens.length; i++) {
            BEP20 token = BEP20(poolParams.constituentTokens[i]);
            require(
    ...
    function joinSmartPool(
        ...
        address[] memory tokens = pool.ePool().getCurrentTokens();
        _join(pool, tokens, poolAmountOut, maxAmountsIn);
    }
    ...
    function finalize(EPool pool) external {
        pool.finalize();
        require(pool.transfer(msg.sender, pool.balanceOf(address(this))), "ERR_TRANSFER_FAILED");
    }
    ...
    function _safeApprove(BEP20 token, address spender, uint amount) internal {
