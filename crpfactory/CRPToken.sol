...
*/
abstract contract PCToken is IBEP20 {
    using EXPXSafeMath for uint;

    // State variables
    string public constant NAME = "EXPX Smart Pool";
    ...
    function approve(address spender, uint amount) external override returns (bool) {
        /* In addition to the increase/decreaseApproval functions, could
           ...
           Some token contracts already revert if you call approve on a non-zero allocation.
           To deal with these, we use the EXPXSafeApprove library and safeApprove function
           when adding tokens to the pool.
        */
    ...
    function increaseApproval(address spender, uint amount) external returns (bool) {
        _allowance[msg.sender][spender] = EXPXSafeMath.eAdd(_allowance[msg.sender][spender], amount);
    ...
    function decreaseApproval(address spender, uint amount) external returns (bool) {
        ...
        if (amount >= oldValue) {
            ...
            _allowance[msg.sender][spender] = EXPXSafeMath.eSub(oldValue, amount);
        }
    ...
    function transferFrom(address sender, address recipient, uint amount) external override returns (bool) {
        ...
        if (msg.sender != sender && oldAllowance != uint(-1)) {
            _allowance[sender][msg.sender] = EXPXSafeMath.eSub(oldAllowance, amount);
    ...
    function _mint(uint amount) internal virtual {
        _balance[address(this)] = EXPXSafeMath.eAdd(_balance[address(this)], amount);
        varTotalSupply = EXPXSafeMath.eAdd(varTotalSupply, amount);
    ...
    function _burn(uint amount) internal virtual {
        ...
        // Remove require for gas optimization - eSub will revert on underflow
        // require(_balance[address(this)] >= amount, "ERR_INSUFFICIENT_BAL");

        _balance[address(this)] = EXPXSafeMath.eSub(_balance[address(this)], amount);
        varTotalSupply = EXPXSafeMath.eSub(varTotalSupply, amount);
    ...
    function _move(address sender, address recipient, uint amount) internal virtual {
        ...
        _balance[sender] = EXPXSafeMath.eSub(_balance[sender], amount);
        _balance[recipient] = EXPXSafeMath.eAdd(_balance[recipient], amount);
