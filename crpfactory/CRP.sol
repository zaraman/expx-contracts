...
import "./CRPToken.sol";
import "./EXPXReentrancyGuard.sol";
import "./EXPXOwnable.sol";

// Interfaces

// Libraries
import { RightsManager } from "./RightsManager.sol";
import "./CRPManager.sol";
 ...
 * @notice PCToken is the "EXPX Smart Pool" token (transferred upon finalization)
 ...
 *      4: canWhitelistLPs - can restrict liquidity providers to a whitelist
 *      5: canChangeCap - can change the esp cap (max # of pool tokens)
 *
 * Note that functions called on ePool and eFactory may look like internal calls,
 *   but since they are contracts accessed through an interface, they are really external.
 * To make this explicit, we could write "IEPool(address(ePool)).function()" everywhere,
 *   instead of "ePool.function()".
 */
contract ConfigurableRightsPool is PCToken, EXPXOwnable, EXPXReentrancyGuard {
    using EXPXSafeMath for uint;
    using EXPXSafeApprove for IBEP20;

    // Type declarations

    struct PoolParams {
        // EXPX Pool Token (representing shares of the pool)
    ...
    //   construction and createPool - thereafter, use the underlying EPool's list
    ...
    modifier needsEPool() {
        require(address(ePool) != address(0), "ERR_NOT_CREATED");
        _;
    }
    ...
    modifier lockUnderlyingPool() {
        ...
        bool origSwapState = ePool.isPublicSwap();
        ePool.setPublicSwap(false);
        _;
        ePool.setPublicSwap(origSwapState);
    }
    ...
    /**
     * @notice Construct a new Configurable Rights Pool (wrapper around EPool)
     ...
     * @param factoryAddress - the EPoolFactory used to create the underlying pool
    ...
    PCToken(poolParams.poolTokenSymbol, poolParams.poolTokenName)
    {
        ...
        require(poolParams.swapFee >= EXPXConstants.MIN_FEE, "ERR_INVALID_SWAP_FEE");
        require(poolParams.swapFee <= EXPXConstants.MAX_FEE, "ERR_INVALID_SWAP_FEE");
        ...
        // Cannot have too many or too few - technically redundant, since EPool.bind() would fail later
        // But if we don't check now, we could have a useless contract with no way to create a pool

        require(poolParams.constituentTokens.length >= EXPXConstants.MIN_ASSET_LIMIT, "ERR_TOO_FEW_TOKENS");
        require(poolParams.constituentTokens.length <= EXPXConstants.MAX_ASSET_LIMIT, "ERR_TOO_MANY_TOKENS");
        // There are further possible checks (e.g., if they use the same token twice), but
        // we can let bind() catch things like that (i.e., not things that might reasonably work)

        SmartPoolManager.verifyTokenCompliance(poolParams.constituentTokens);

        eFactory = IEFactory(factoryAddress);
        rights = rightsStruct;
        ...
        espCap = EXPXConstants.MAX_UINT;
    }

    // External functions

    /**
     * @notice Getter for bep token owner
     ...
     *      ePool is a contract interface; function calls on it are external
    ...
    returns (bool)
    {
        return ePool.isPublicSwap();
    }

    /**
     * @notice Set the cap (max # of pool tokens)
     * @dev _espCap defaults in the constructor to unlimited
    ...
    onlyOwner
    {
        ...
        emit CapChanged(msg.sender, espCap, newCap);
    ...
    virtual
    {
        ...
        ePool.setPublicSwap(publicSwap);
    }
     ...
     * @dev Notice Balance is not an input (like with rebind on EPool) since we will require prices not to change
    ...
    */
    ...
    virtual
    {
        ...
        SmartPoolManager.updateWeight(IConfigurableRightsPool(address(this)), ePool, token, newWeight);
    }
     ...
     *                     You can get the underlying EPool (it's public), and call
    ...
    */
    ...
    virtual
    {
        ...
        // Delegate to library to save space
        SmartPoolManager.pokeWeights(ePool, gradualUpdate);
    }
    ...
    * @dev ePool is a contract interface; function calls on it are external
    ...
    */
    ...
    needsEPool
    {
        ...
        require(rights.canAddRemoveTokens,"ERR_CANNOT_ADD_REMOVE_TOKENS");
        // After createPool, token list is maintained in the underlying EPool
        ...
        // Delegate to library to save space
        SmartPoolManager.removeToken(IConfigurableRightsPool(address(this)), ePool, token);
    }
    ...
    lockUnderlyingPool
    {
        ...
        // After createPool, token list is maintained in the underlying EPool
        address[] memory poolTokens = ePool.getCurrentTokens();
    ...
    lockUnderlyingPool
    {
        ...
        _pushPoolShare(address(eFactory), exitFee);
    ...
    returns (uint tokenAmountOut)
    {
        ...
        uint pAiAfterExitFee = EXPXSafeMath.eSub(poolAmountIn, exitFee);
    ...
    */
    ...
    returns (uint)
    {
        return ePool.getDenormalizedWeight(token);
    }
    ...
    */
    ...
    /**
     * @notice Getter for the EXPXSafeMath contract
     * @dev Convenience function to get the address of the EXPXSafeMath library (so clients can check version)
     * @return address of the EXPXSafeMath library
    */
    function getEXPXSafeMathVersion() external pure returns (address) {
        return address(EXPXSafeMath);
    }
    ...
    */
    ...
    function createPoolInternal(uint initialSupply) internal {
        require(address(ePool) == address(0), "ERR_IS_CREATED");
        require(initialSupply >= EXPXConstants.MIN_POOL_SUPPLY, "ERR_INIT_SUPPLY_MIN");
        require(initialSupply <= EXPXConstants.MAX_POOL_SUPPLY, "ERR_INIT_SUPPLY_MAX");
        ...
        if (rights.canChangeCap) {
            espCap = initialSupply;
        }

        // There is technically reentrancy here, since we're making external calls and
        // then transferring tokens. However, the external calls are all to the underlying EPool

        // To the extent possible, modify state variables before calling functions
        _mintPoolShare(initialSupply);
        _pushPoolShare(msg.sender, initialSupply);

        // Deploy new EPool (eFactory and ePool are interfaces; all calls are external)
        ePool = eFactory.newEPool();

        // EXIT_FEE must always be zero, or ConfigurableRightsPool._pushUnderlying will fail
        require(ePool.EXIT_FEE() == 0, "ERR_NONZERO_EXIT_FEE");
        require(EXPXConstants.EXIT_FEE == 0, "ERR_NONZERO_EXIT_FEE");
        ...
        for (uint i = 0; i < _initialTokens.length; i++) {
            ...
            bool returnValue = IBEP20(t).transferFrom(msg.sender, address(this), bal);
            require(returnValue, "ERR_BEP20_FALSE");

            returnValue = IBEP20(t).safeApprove(address(ePool), EXPXConstants.MAX_UINT);
            require(returnValue, "ERR_BEP20_FALSE");

            ePool.bind(t, bal, denorm);
        }
        ...
        ePool.setSwapFee(_initialSwapFee);
        ePool.setPublicSwap(true);
    ...
    // Rebind EPool and pull tokens from address
    // ePool is a contract interface; function calls on it are external
    function _pullUnderlying(address bep20, address from, uint amount) internal needsEPool {
        // Gets current Balance of token i, Bi, and weight of token i, Wi, from EPool.
        uint tokenBalance = ePool.getBalance(bep20);
        uint tokenWeight = ePool.getDenormalizedWeight(bep20);

        bool xfer = IBEP20(bep20).transferFrom(from, address(this), amount);
        require(xfer, "ERR_BEP20_FALSE");
        ePool.rebind(bep20, EXPXSafeMath.eAdd(tokenBalance, amount), tokenWeight);
    }

    // Rebind EPool and push tokens to address
    // ePool is a contract interface; function calls on it are external
    function _pushUnderlying(address bep20, address to, uint amount) internal needsEPool {
        // Gets current Balance of token i, Bi, and weight of token i, Wi, from EPool.
        uint tokenBalance = ePool.getBalance(bep20);
        uint tokenWeight = ePool.getDenormalizedWeight(bep20);
        ePool.rebind(bep20, EXPXSafeMath.eSub(tokenBalance, amount), tokenWeight);

        bool xfer = IBEP20(bep20).transfer(to, amount);
        require(xfer, "ERR_BEP20_FALSE");
    }
    ...
    function _mint(uint amount) internal override {
        super._mint(amount);
        require(varTotalSupply <= espCap, "ERR_CAP_LIMIT_REACHED");
    }
